import { toJSON, toDynamoJSON } from '../src/util.js'
import fs from 'node:fs'
import { test } from 'node:test'
import * as assert from 'node:assert'

test('create dynamo json from regular json', async t => {
  const o = {
    str: 'string',
    int: 1,
    float: 1.1,
  ...
  assert.ok(expected === JSON.stringify(j))
  assert.ok(JSON.stringify(toJSON(j)) === JSON.stringify(o))
})

test('large object', async t => {
  const o = JSON.parse(fs.readFileSync('test/fixture.json'))
